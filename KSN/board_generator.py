...
def profit(combined_graph, graph_i, i, j, results, diffusion='LT'):
    S, SPREAD, _, _, _ = celf(deepcopy(combined_graph), graph_i, j, diffusion)
    results[i] = [S, SPREAD]


def board_generator(combined_graph, graphs, l, diffusion='LT'):
    """
    ...
    for i in range(len(graphs)):
        graph_i = graphs[i]
        p = Process(target=profit, args=(combined_graph, graph_i, i, l, results, diffusion))
        processes.append(p)
        p.start()
