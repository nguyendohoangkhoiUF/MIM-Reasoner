...
# def IC_rl(combined_graph, T, node_active_graph, mc=1):
#     """
#         Input:
...
def celf(combined_graph, graph, good_nodes, k, diffusion='LT'):
    """
    ...
    for node in tqdm(graph.nodes()):
        if node in good_nodes:
            ...
            data.append(node_status)
            
            s = sum(combined_graph.nodes[i]['attribute'] for i, node_x in enumerate(node_status) if node_x > 0)
            marg_gain.append(s)
