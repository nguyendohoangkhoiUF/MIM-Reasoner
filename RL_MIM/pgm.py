...
def get_pgm(node_status, seed_set, combined_graph, thresold=0.5):
    ...
    variables = list(set(model_exact_infer.variables) - set(seed_set))

    evidence = {node: 2 for node in seed_set}
    node_pgm = [1 for node in combined_graph.nodes()]
    for node in variables:
        ...
        for node_corr in groups[node]:
            node_pgm[node_corr] = max(0, 1 - 4 * tasty_infer.values[1])
